...
Ran a python script to obtain these results, will use your code, for this question
...
#+BEGIN_SRC python :results output
  ...
  : The random sentence retracted from the Twitter Corpus is: Well go win some money :) Good or lucky, I don't care
...
39. [@39]  <<bo-tagger>> Construct a lookup tagger trained on the 1000 most frequent words in
    ...
    #+BEGIN_SRC python :results output
      ...
      #this tagger will tag all words with unk by default
      default_tagger = nltk.DefaultTagger('UNK')

      #This will load the news category of Brown
      tagged_words = brown.tagged_words(categories='news')

      from nltk.probability import FreqDist

      #creates a frequency distribution of the words in news category
      frequent_dist = FreqDist(tagged_words)

      '''for word, frequency in freq_dist.most_common(100):
	  print(f'{word} : {frequency}')'''

      #The top 1000 common word-tag pairs
      top_frequency_dist = frequent_dist.most_common(1000)
      token_tag_tuples = [tup for tup, frequency in top_frequency_dist]
      ...
      #Select a random sentence and then tag the random sentence
      random_integer = random.randint(0, len(sentences))
      rand_sentence = sentences[random_integer]

      print(unigram_tagger.tag(rand_sentence))


#+END_SRC

#+RESULTS:
: [('The', 'AT'), ('funding', 'UNK'), ('is', 'BEZ'), ('attached', 'UNK'), ('*-11', 'UNK'), ('to', 'TO'), ('an', 'AT'), ('estimated', 'UNK'), ('$', 'UNK'), ('27.1', 'UNK'), ('billion', 'CD'), ('*U*', 'UNK'), ('transportation', 'UNK'), ('bill', 'NN'), ('that', 'CS'), ('*T*-1', 'UNK'), ('goes', 'UNK'), ('*ICH*-2', 'UNK'), ('next', 'AP'), ('to', 'TO'), ('the', 'AT'), ('Senate', 'NN-TL'), ('and', 'CC'), ('carries', 'UNK'), ('with', 'IN'), ('it', 'PPS'), ('a', 'AT'), ('proposed', 'VBN'), ('permanent', 'UNK'), ('smoking', 'UNK'), ('ban', 'UNK'), ('on', 'IN'), ('virtually', 'UNK'), ('all', 'ABN'), ('U.S.', 'NP'), ('domestic', 'UNK'), ('airline', 'UNK'), ('flights', 'UNK'), ('.', '.')]
...
In the results I notice that many words are tagged as 'UNK'. Most of the words are tagged that way because the unigram tagger is trained on the Brown
news corpus. Twitter language uses a lot of slang that are not present in the Brown corpus. The vocabulary from the two corpus' are very
different. A unigram tagger is suitable for texts that are close to its data for training but not great with ones that are different. 
...
#+RESULTS:
: The number of sentences in the corpus appears to be: 26618
: The random sentence retracted from the Twitter Corpus is: I haven't ever given the impression that i would EVER consider voting Tory, have I? 😮 @RSimpsonMSP maybe she could remember one from her Tory campaign? RT @Bonn1eGreer: "Ed  slipped on #DavidCameron 's sweat

#  [[to-gram]], 40
42.  [@42] What do you observe in your results for question [[to-gram]]?  Why do you think
    this is happening?

I noticed that for all tokens that do not match the grammar, then the parser gives a single leaf that is not part of the chunk.
Else, a chunk is given with the word to and then the following token is either a noun or a type of verb. There does not appear to be
any interesting patterns to the eye. 
